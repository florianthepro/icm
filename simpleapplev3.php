<?php
/**
 * simple-apple-v3.php - iCloud-Postfach von einer Claude-Routine sortieren lassen.
 *
 ...
 * Cron alle fuenf Minuten:  php /pfad/simple-apple-v3.php cron
 */
...
function sa_fragen_sammeln(array $user, bool $initial): array
{
    try {
        ...
        foreach ($quellen as $f) {
            ...
            if ($f['name'] === 'INBOX' && !$initial) {
                $marke = ($user['uidvalidity'] ?? '') === $imap->uidvalidity
                    ? (int) ($user['last_uid'] ?? 0) : 0;
                $uids = array_values(array_filter($uids,
                    static fn($u) => (int) $u > $marke));
            }
            foreach ($imap->headers($uids) as $m) {
                ...
                $kind = alias_of($m['to'], $user) === null ? 'primary' : 'alias';
...
function sa_sortieren(string $id, bool $initial): array
{
    ...
    try {
        ...
        if ($initial) {
            // Alles in den Posteingang, dann die eigenen Ordner loeschen.
            // Systemordner bleiben stehen, nur das Archiv wird geleert -
            // Gesendet, Entwuerfe, Werbung und Papierkorb bleiben unberuehrt.
            foreach (sa_eigene($imap->folders()) as $f) {
        ...
        $alle = $imap->headers($uids);
        $marke = (!$initial && ($user['uidvalidity'] ?? '') === $imap->uidvalidity)
            ? (int) ($user['last_uid'] ?? 0) : 0;
        $neu = array_values(array_filter($alle,
            static fn(array $m) => (int) $m['uid'] > $marke));
        $r['gelesen'] = count($neu);
        ...
        foreach ($neu as $m) {
            ...
            $ordner = is_string($wohin) ? resolve_answer($wohin, $pfad) : null;
            if ($ordner !== null && folder_shape_ok($ordner)) {
                $moves[$ordner][] = $m['uid'];
            } else {
                $offen++;
            }
        ...
        $hoch = 0;
        foreach ($alle as $m) {
            $hoch = max($hoch, (int) $m['uid']);
        }
        $weiter = $offen === 0;
        $uidvalidity = $imap->uidvalidity;
        user_set($id, static function (array $u) use ($weiter, $hoch, $uidvalidity,
                                                      $benutzt, $initial): array {
            $u['uidvalidity'] = $uidvalidity;
            if ($weiter) {
                $u['last_uid'] = max((int) ($u['last_uid'] ?? 0), $hoch);
            }
...
function sa_schritt(string $id): array
{
    ...
    return [sprintf('%d Adressen an die Routine geschickt. Die Antwort kommt gleich zurueck.',
        count($zeilen)), null];
}
...
function sa_job_finden(string $token): array
{
    ...
    foreach (users() as $u) {
        $job = $u['job'] ?? null;
        if (is_array($job) && hash_equals((string) ($job['token_hash'] ?? ''), $hash)) {
            return [$u, $job];
        }
    }
    return [null, null];
...
function sa_rueckweg_bedienen(string $token): never
{
    ...
    $ersatz = array_key_exists('Persoenlich', $katalog) ? 'Persoenlich' : array_key_first($katalog);
    $offen = 0;
    foreach ($zeilen as $z) {
        if (!isset($wissen[(string) $z['kind']][(string) $z['von']])) {
            $wissen[(string) $z['kind']][(string) $z['von']] = $ersatz;
            $offen++;
        }
    }
...
if (PHP_SAPI === 'cli') {
    ($argv[1] ?? '') === 'cron' ? sa_cron() : bail('Aufruf: php simple-apple-v3.php cron');
}
